...
int main()
{
  ...
  worker[0].call(std::bind(testFunc0),-1);

  worker[0].callAsync(std::bind(testFunc0),-1);
  worker[0].sync(-1);
  int a=1;
  worker[0].call(std::bind(testFunc1,a),-1);
  int b=2;
  worker[0].call(std::bind(testFunc2,a,std::ref(b)),-1);
  if(b != 3)
    return EXIT_FAILURE;
